...
const writeFile = (file, data, reporter) => {
  ...
  return new Promise((resolve) => {
    fs.writeFile(file, data, (err) => {
      if (err) {
        reporter.warn(
          `301 Redirects: Unable to write to redirects file - ${file}`
        );
      }

      reporter.success(
        `301 Redirects: Successfully wrote redirects to file - ${file}`
      );
...
const readFile = (file, reporter) => {
  ...
  return new Promise((resolve) => {
    fs.readFile(file, 'utf8', (err, data = '') => {
      if (err) {
        reporter.warn(
          `301 Redirects: Unable to read existing redirects from file - ${file}`
        );
...
exports.onPostBuild = async function onPostBuild(helpers, options) {
  ...
  try {
    ...
    const newRedirects = redirectNodes.reduce(
      (redirects, { to, from }) => `${redirects}\n${from} ${to}`,
      ''
    );
    ...
    reporter.info(
      `301 Redirects: Found ${existingRedirectsCount} existing redirects`
    );
    reporter.info(`301 Redirects: Found ${newRedirectsCount} new redirects`);
    reporter.info(
      `301 Redirects: Writing out ${
        existingRedirectsCount + newRedirectsCount
      } total redirects`
    );
