...
const {
  fetchFreshCollectionData,
  sortUnchangedRemovedAndStaleNodes,
} = require('./src/fetch');
const {
  touchUnchangedNodes,
  deleteRemovedNodes,
  createCollectionNodes,
  createCollectionImageNodes,
} = require('./src/nodes');
...
exports.sourceNodes = async (helpers, options) => {
  ...
  if (!accessToken) {
    panic(
      `Please include an Shopify storefront access token to ${PLUGIN_NAME}.`
    );
  }
  ...
  const { unchangedNodes, removedNodes, staleNodes } =
  ...
  if (staleNodes.length > 0) {
    ...
    const collections = await fetchFreshCollectionData({
      staleNodes,
      client,
      helpers,
    });
  ...
  timer.setStatus(
    format`Sourced {bold BackpackCollection} nodes from {green {bold Shopify}}`
  );
...
exports.createSchemaCustomization = ({
  ...
  const hasRequiredTypes = requiredTypes.every(
    (requiredType) => getNodesByType(requiredType).length > 0
  );
