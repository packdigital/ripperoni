...
const diffAndUpdateNodes = ({
  ...
  return ({
    ...
    const newNodes = getNewData(data.result, type, helpers).map(createAndCacheNode(type, helpers));
    const updatedNodes = getUpdatedData(data.result, type, helpers).map(createAndCacheNode(type, helpers));
    const deletedNodes = getDeletedNodes(data.result, type, helpers).map(deleteAndUncacheNode(helpers));
...
const touchUnchangedCachedData = async ({
  client,
  shopId,
  helpers
}) => {
...
const queryForNewNodes = async ({
  ...
  timer.end();
  Object.entries(data).map(([type, result]) => diffAndUpdateNodes({
    type,
    helpers
  })({
    data: {
...
exports.createContentNodes = async ({
  client,
  shopId,
  helpers
}) => {
  try {
    await touchUnchangedCachedData({
      client,
      shopId,
      helpers
    });
    await queryForNewNodes({
      client,
      shopId,
      helpers
    });
