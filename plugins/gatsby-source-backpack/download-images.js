...
exports.downloadImages = async ({
  ...
  const downloadedImages = getNodesByType(prefixedImage).map(node => downloadImageAndCreateRemoteFileNode(node, helpers, createRemoteFileNode, TYPE_PREFIX)); // eslint-disable-next-line no-undef

  const images = await Promise.all(downloadedImages).then(images => images.filter(image => image));
  const cachedFiles = images.filter(({
    type
  }) => type === 'TOUCH_NODE');
  const downloadedFiles = images.filter(({
    __typename
  }) => __typename === 'Image');
