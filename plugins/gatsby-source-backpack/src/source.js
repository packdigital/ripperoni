...
exports.touchUnchangedCachedData = async ({ client, shopId, helpers }) => {
  try {
    ...
    const touchedTypes = Object.entries(backpackMeta.data).map(
      async ([type, data]) => {
        let touchedNodes = 0;

        const nodes = data.map(async (data) => {
          const backpackId = convertToGatsbyGraphQLId(
            data.id,
            type,
            TYPE_PREFIX
          );
        ...
        if (touchedNodes > 0) {
          success(
            format`{${LOG_PREFIX}} Load {bold ${TYPE_PREFIX}${type}} from cache - {bold ${touchedNodes} nodes}`
          );
        }
    ...
    helpers.reporter.panic(
      'Something went wrong while touching cached Backpack Nodes: ',
      error
    );
  }
};
...
exports.queryAndCreateNewNodes = async ({ client, shopId, helpers }) => {
  try {
    ...
    const timer = activityTimer(
      format`{${LOG_PREFIX}} Fetch new node data from {red {bold Backpack}}`
    );
    ...
    helpers.reporter.panic(
      'Something went wrong while sourcing and creating new Backpack Nodes: ',
      error
    );
  }
};
...
exports.setupSubscriptions = async ({ client, shopId, helpers }) => {
  try {
        ...
        .subscribe({
          ...
          error: (error) =>
            helpers.reporter.error(
              'Something went wrong while recieving data from Backpack Subscription',
              error
            ),
        });
    });
    ...
    helpers.reporter.panic(
      'Something went wrong while setting up subscriptions for Backpack Nodes: ',
      error
    );
  }
};
