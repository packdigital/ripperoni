...
const createProductJson = (store, graphql, reporter) => async ({ template, query, previewable = true, jsonKey }) => {
  ...
  data.allNodes.nodes.forEach(async(node) => {
    ...
    const fileNamePath = `${program.directory}/static/${template}/${jsonKey}/${fileName}.js`;

    // Create the directory for the static data
    try {
      await fs.mkdirp(path.join(__dirname, pathName));
    } catch (error) {
    ...
    try {
      await fs.writeFile(
        path.join(__dirname, fileNamePath),
...
module.exports = createProductJson;