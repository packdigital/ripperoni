...
const _getNewData = (freshData, type, helpers) => {
  ...
  return freshData.filter(
    (data) =>
      !oldNodeIds.includes(convertToGatsbyGraphQLId(data.id, type, TYPE_PREFIX))
  );
};
...
exports.diffAndUpdateNodes = ({ type, helpers }) => {
  return ({ data }) => {
    ...
    getNodesByType(prefixedType).forEach((node) =>
      touchNode({ nodeId: node.id })
    );

    const newNodes = _getNewData(data.result, type, helpers).map(
      _createAndCacheNode(type, helpers)
    );

    const updatedNodes = _getUpdatedData(data.result, type, helpers).map(
      _createAndCacheNode(type, helpers)
    );

    const deletedNodes = _getDeletedNodes(data.result, type, helpers).map(
      _deleteAndUncacheNode(helpers)
    );
