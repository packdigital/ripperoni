...
exports.diffAndUpdateNodes = ({ type, helpers }) => {
  ...
  return ({ data }) => {
    ...
    const newNodes = _getNewData(data.result, type, helpers)
      .map(_createAndCacheNode(type, helpers));

    const updatedNodes = _getUpdatedData(data.result, type, helpers)
      .map(_createAndCacheNode(type, helpers));

    const deletedNodes = _getDeletedNodes(data.result, type, helpers)
      .map(_deleteAndUncacheNode(helpers));
