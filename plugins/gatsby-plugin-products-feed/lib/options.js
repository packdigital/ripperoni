...
exports.feedOptions = ({ pluginOptions, reporter }) => {
  ...
  if (!pluginOptions.shopifyDomain) {
    reporter.panic('You must provide a [shopifyDomain] option. i.e "public-rec-headless.myshopify.com"');
  }
  if (!pluginOptions.publicUrl) {
    reporter.panic('You must provide a [publicUrl] option. i.e "https://publicrec.com"');
  }

  // Merge feedItem arrays
  const mergedFeedItems = [...pluginOptions.feedItem, ...defaultOptions.feedItem];
